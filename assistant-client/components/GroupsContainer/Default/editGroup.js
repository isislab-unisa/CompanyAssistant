...
import { edit } from "../../../controllers/groupController";
...
export default class EditGroup extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({ tags: this.props.group.tags })
    this.setState({ selectedTags: this.props.group.tags })
  }



  editGroupHandler(newGroup) {
      ...
      <>
        ...
        <DialogTitle id="responsive-dialog-title">
          <Grid container direction="row" style={{
            ...
            <Grid item><Avatar {...stringAvatar(this.props.group.name)} /> </Grid>
            <Grid Item style={{ verticalAlign: "middle", fontSize: "1.8em" }}> &nbsp; {this.props.group.name}</Grid>
          </Grid>
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
                  ...
                  <form style={{ width: "100%" }} id="formTag">
                    <FormGroup controlId="formBasicCredentials" style={{ margin: "5px 0 10px 0" }}>
                      ...
                      <div style={{ width: "100%", padding: "1em", display: "inline-flex", verticalAlign: "middle", overflow: "auto" }}>
                        <TextField required id="outlined-required" name="newTag" type="text" onChange={e => this.setState({ newTag: e.target.value })} label="newTag" disabled={!this.props.policies.includes(UPDATE_GROUP_POLICY) && store.getState().auth.userType != "admin"} style={{ margin: "2px 5% 20px 0", width: "70%" }} />
                        <Button style={{ width: "20%", height: "40px", marginTop: "0.5em" }} size="large" variant="outlined" disabled={!this.props.policies.includes(UPDATE_GROUP_POLICY) && store.getState().auth.userType != "admin"} type="button" onClick={insertTagHandler}>
                  ...
                  <div style={{ width: "100%", display: "inline-flex", overflow: "auto", marginBottom: "30px", padding: "1rem 0" }}>
                    ...
                    >
                      {this.state.tags !== undefined ?
                        this.state.tags.map(
                          (item, index) => (<ToggleButton aria-label={item} disabled={!this.props.policies.includes(UPDATE_GROUP_POLICY) && store.getState().auth.userType != "admin"} value={item}>{item}</ToggleButton>)
                        ) : null
                      }
                      {
        ...
        <DialogActions>
          ...
          <Button disabled={!this.props.policies.includes(UPDATE_GROUP_POLICY) && store.getState().auth.userType != "admin"} variant="primary" onClick={this.state.activeStep === steps.length ? () => { this.editGroupHandler(this.state) } : this.handleOnClickNext}>
